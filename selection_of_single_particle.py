    ...
    for p in range(0, Nb_particles):
        ...
        if len(t) == 1:
            if (t[t.frame_break == N].mean_value_before_break >= seuil).bool():
                ...
                df0.loc[i, "mean_value_before_break"] = df1.loc[
                    u, "mean_value_before_break"
                ]
                df0.loc[i, "mean_value_after_break"] = df1.loc[
                    u, "mean_value_before_break"
                ]
        ...
        if len(t) == 2:  # 1 rupture
            ...
            if df2.loc[u + 1, "mean_value_before_break"] < seuil:
                ...
                df0.loc[i, "mean_value_after_break"] = df2.loc[
                    u + 1, "mean_value_before_break"
                ]
                ...
                if (
                    (
                        df0.loc[i, "mean_value_before_break"]
                        > df0.loc[i, "mean_value_after_break"]
                    )
                    and (df0.loc[i, "mean_value_before_break"] > seuil)
                    and (epsilon2 < 0.5 * ecarttype)
                    and (epsilon1 > 0.5 * ecarttype)
                ):
                    i = i + 1
                    k = k + 1
