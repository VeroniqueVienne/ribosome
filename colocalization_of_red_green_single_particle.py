...
def colocalization(dfr, dfg, radius=3)->(float, float):
    """Detect proportion of red and green particles colocalized at Frame 0."""
    ...
    percentage_red = len(dfr1) / len(dfr) * 100
    percentage_green = len(dfg1) / len(dfg) * 100

    print(
        "percentage of colocalization for red particles=",
        len(dfr1) / len(dfr) * 100,
        "%",
    )
    print(
        "percentage of colocalization for green particles=",
        len(dfg1) / len(dfg) * 100,
        "%",
    )
    ...
    for itemr, rowr in dfr.iterrows():  # each line is a different fluorophore
        ...
        for itemg, rowg in dftest.iterrows():
            if dftest.loc[itemg, "x"] < radius:
                ...
                df.loc[i, "Dt"] = (
                    dfr.loc[itemr, "frame_break"] - dfg.loc[itemg, "frame_break"]
                )
                df.loc[i, "mean_value_before_break"] = dfr.loc[
                    itemr, "mean_value_before_break"
                ]
                ...
                df.loc[i + 1, "Dt"] = (
                    dfg.loc[itemg, "frame_break"] - dfr.loc[itemr, "frame_break"]
                )
                df.loc[i + 1, "mean_value_before_break"] = dfg.loc[
                    itemg, "mean_value_before_break"
                ]

                if (
                    df.loc[i, "frame_break"] == Nbframe
                    or df.loc[i + 1, "frame_break"] == Nbframe
                ):
